	...
	for (i = 0; i < entries; i++) {
		...
		if (dentries[i].filename == NULL)
			critical_error_errno("strdup");
                ...
                unused = asprintf(&dentries[i].path, "%s/%s", dir_path, namelist[i]->d_name);
                unused = asprintf(&dentries[i].full_path, "%s/%s", full_path, namelist[i]->d_name);
		...
		if (S_ISREG(stat.st_mode)) {
			...
			dentries[i].link = calloc(info.block_size, 1);
                        unused = readlink(dentries[i].full_path, dentries[i].link, info.block_size - 1);
		} else {
